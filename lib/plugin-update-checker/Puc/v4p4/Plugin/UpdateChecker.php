...
if (! class_exists('Puc_v4p4_Plugin_UpdateChecker', false)) :
    ...
    class Puc_v4p4_Plugin_UpdateChecker extends Puc_v4p4_UpdateChecker
    {
        ...
        public function __construct($metadataUrl, $pluginFile, $slug = '', $checkPeriod = 12, $optionName = '', $muPluginFile = '')
        {
            ...
            add_filter($slugCheckFilter, array(
                $this,
                'getAbsolutePath'
            ));
            ...
            add_action('uninstall_' . $this->pluginFile, array(
                $this,
                'removeHooks'
            ));
        ...
        {
            $scheduler = new Puc_v4p4_Scheduler($this, $checkPeriod, array(
                'load-plugins.php'
            ));
            register_deactivation_hook($this->pluginFile, array(
                $scheduler,
                'removeUpdaterCron'
            ));
            return $scheduler;
        ...
        public function removeHooks()
        {
            ...
            remove_filter('plugins_api', array(
                $this,
                'injectInfo'
            ), 20);
            ...
            remove_action('delete_site_transient_update_plugins', array(
                $this,
                'clearCachedVersion'
            ));
        }
        ...
        public function getInstalledVersion()
        {
            ...
            if (isset($pluginHeader['Version'])) {
                ...
                $this->triggerError(sprintf("Can't to read the Version header for '%s'. The filename is incorrect or is not a plugin.", $this->pluginFile), E_USER_WARNING);
                return null;
            }
        }
        ...
        protected function getPluginHeader()
        {
            if (! is_file($this->pluginAbsolutePath)) {
                ...
                $this->triggerError(sprintf("Can't to read the plugin header for '%s'. The file does not exist.", $this->pluginFile), E_USER_WARNING);
                return array();
            }
         ...
         * directory.
         * Example: "plugin-name/plugin.php".
         *
         * @return string
         */
         ...
         *            The upgrader that's performing the current update.
         * @return bool
         */
        ...
        public function addCheckForUpdatesLink($pluginMeta, $pluginFile)
        {
            ...
            if ($isRelevant && $this->userCanInstallUpdates()) {
                $linkUrl = wp_nonce_url(add_query_arg(array(
                    'puc_check_for_updates' => 1,
                    'puc_slug' => $this->slug
                ), self_admin_url('plugins.php')), 'puc_check_for_updates');

                $linkText = apply_filters($this->getUniqueName('manual_check_link'), __('Check for updates', 'plugin-update-checker'));
                if (! empty($linkText)) {
                    /**
                     *
                    ...
                    if ($pluginData['PluginURI']) {
                        ...
                        foreach ($pluginMeta as $linkIndex => $existingLink) {
                            if (strpos($existingLink, $escapedPluginUri) !== false) {
                                ...
                                $viewDetailsLinkPosition = apply_filters($this->getUniqueName('view_details_link_position'), 'before');
                                break;
                            }
                        }
                    }

                    $viewDetailsLink = sprintf('<a href="%s" class="thickbox open-plugin-details-modal" aria-label="%s" data-title="%s">%s</a>', esc_url(network_admin_url('plugin-install.php?tab=plugin-information&plugin=' . urlencode($this->slug) . '&TB_iframe=true&width=600&height=550')), esc_attr(sprintf(__('More information about %s'), $pluginData['Name'])), esc_attr($pluginData['Name']), $linkText);
        ...
        {
            $shouldCheck = isset($_GET['puc_check_for_updates'], $_GET['puc_slug']) && $_GET['puc_slug'] == $this->slug && $this->userCanInstallUpdates() && check_admin_referer('puc_check_for_updates');
         ...
         *
         * @see self::handleManualCheck() You can change the result message by using the "puc_manual_check_message-$slug" filter.
         */
        public function displayManualCheckResult()
        {
            if (isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug)) {
                ...
                printf('<div class="notice %s is-dismissible"><p>%s</p>%s</div>', $noticeClass, apply_filters($this->getUniqueName('manual_check_message'), $message, $status), $details);
            }
        }
        ...
        /**
         * Clear the cached plugin version.
         * This method can be set up as a filter (hook) and will
