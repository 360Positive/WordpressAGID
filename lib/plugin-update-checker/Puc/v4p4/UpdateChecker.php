...
if (! class_exists('Puc_v4p4_UpdateChecker', false)) :
    ...
    abstract class Puc_v4p4_UpdateChecker
    {
         ...
         * Errors are raised using trigger_error()
        ...
        protected function installHooks()
        {
            ...
            add_filter('site_transient_' . $this->updateTransient, array(
                $this,
                'injectUpdate'
            ));
        ...
        {
            remove_filter('site_transient_' . $this->updateTransient, array(
                $this,
                'injectUpdate'
            ));
            remove_filter('site_transient_' . $this->updateTransient, array(
            ...
            remove_filter('http_request_host_is_external', array(
                $this,
                'allowMetadataHost'
            ), 10);
        ...
        public function checkForUpdates()
        {
            ...
            add_action('puc_api_error', array(
                $this,
                'collectApiErrors'
            ), 10, 4);
            ...
            remove_action('puc_api_error', array(
                $this,
                'collectApiErrors'
            ), 10);
        ...
        /**
         * Reset update checker state - i.e.
         * last check time, cached update data and so on.
         *
         ...
         * @param array|WP_Error|null $httpResult
         *            The value returned by wp_remote_get(), if any.
         ...
         * The list is always an associative array, but the key is different
         ...
         *            Parse the JSON as an instance of this class. It must have a static fromJson method.
        ...
        /**
         * Filter callback.
         * Keeps only translations that *don't* match this plugin or theme.
         *
         ...
         *        
         * @param string $source
         *            The directory to copy to /wp-content/plugins or /wp-content/themes. Usually a subdirectory of $remoteSource.
        ...
        /**
         * Check for incorrect update directory structure.
         * An update must contain a single directory,
