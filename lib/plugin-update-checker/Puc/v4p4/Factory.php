...
if (! class_exists('Puc_v4p4_Factory', false)) :
    ...
    class Puc_v4p4_Factory
    {
         ...
         *            The URL of the metadata file, a GitHub repository, or another supported update source.
         * @param string $fullPath
         *            Full path to the main plugin file or to the theme directory.
         * @param string $slug
         *            Custom slug. Defaults to the name of the main plugin file or the theme directory.
         * @param int $checkPeriod
         *            How often to check for updates (in hours).
         * @param string $optionName
         *            Where to store book-keeping info about update checks.
         * @param string $muPluginFile
         *            The plugin filename relative to the mu-plugins directory.
        ...
        public static function buildUpdateChecker($metadataUrl, $fullPath, $slug = '', $checkPeriod = 12, $optionName = '', $muPluginFile = '')
        {
            ...
            if ($checkerClass === null) {
                trigger_error(sprintf('PUC %s does not support updates for %ss %s', htmlentities(self::$latestCompatibleVersion), strtolower($type), $service ? ('hosted on ' . htmlentities($service)) : 'using JSON metadata'), E_USER_ERROR);
                return null;
            }
            ...
            if (! isset($apiClass)) {
                ...
                if ($apiClass === null) {
                    trigger_error(sprintf('PUC %s does not support %s', htmlentities(self::$latestCompatibleVersion), htmlentities($service)), E_USER_ERROR);
                    return null;
                }
        ...
        protected static function isPluginFile($absolutePath)
        {
            ...
            if (function_exists('get_file_data')) {
                $headers = get_file_data($absolutePath, array(
                    'Name' => 'Plugin Name'
                ), 'plugin');
        ...
        /**
         * Sort available class versions in descending order (i.e.
         * newest first).
         */
        protected static function sortVersions()
        {
            foreach (self::$classVersions as $class => $versions) {
                uksort($versions, array(
                    __CLASS__,
                    'compareVersions'
                ));
