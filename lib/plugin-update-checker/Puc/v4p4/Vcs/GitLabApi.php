...
if (! class_exists('Puc_v4p4_Vcs_GitLabApi', false)) :
    ...
    class Puc_v4p4_Vcs_GitLabApi extends Puc_v4p4_Vcs_Api
    {
         ...
         *
         * @param string $ref
         *            Reference name (e.g. branch or tag).
         * @return string|null
         */
        public function getLatestCommitTime($ref)
        {
            $commits = $this->api('/:user/:repo/repository/commits/', array(
                'ref_name' => $ref
            ));
        ...
        {
            $response = $this->api('/:user/:repo/repository/files/' . $path, array(
                'ref' => $ref
            ));
        ...
        {
            $url = sprintf('https://%1$s/%2$s/%3$s/repository/%4$s/archive.zip', $this->repositoryHost, urlencode($this->userName), urlencode($this->repositoryName), urlencode($ref));
