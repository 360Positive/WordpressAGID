...
<div class="tribe-venue-widget-wrapper">
		...
		<ul class="tribe-venue-widget-list">
			<?php while ( $events->have_posts() ): ?>
				...
				<li class="<?php tribe_events_event_classes() ?>">
					<?php
        if (tribe('tec.featured_events')->is_featured(get_the_ID()) && get_post_thumbnail_id(get_the_ID())) {
            /**
        ...
        ?>
				...
				<a href="<?php echo esc_url( tribe_get_event_link() ); ?>"><?php echo get_the_title( get_the_ID() ) ?></a>
					...
					<?php if ( tribe_get_cost( get_the_ID() ) != '' ): ?>
						<span class="tribe-events-divider">|</span> <span
			class="tribe-events-event-cost">
	...
	<a
		href="<?php echo esc_url( tribe_get_venue_link( $venue_ID, false ) ); ?>"><?php printf( __( 'View all %1$s at this %2$s', 'tribe-events-calendar-pro' ), $events_label_plural, tribe_get_venue_label_singular() ); ?></a>
</div>
